      ...
      <header className="flex-none z-40 bg-[#0A0A0B]/95 backdrop-blur-md px-4 py-3 border-b border-white/[0.04] flex items-center justify-between relative">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button 
            ...
            className="flex items-center justify-center transition-colors hover:text-zinc-300 active:scale-90 p-2 -ml-2 shrink-0 relative z-[100] cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button onClick={() => onOpenLiveModal && onOpenLiveModal()} className="px-2.5 py-1 bg-rose-500/10 border border-rose-500/30 text-rose-500 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1 hover:bg-rose-500/20 transition-colors">
        ...
        <div className="flex flex-col w-full pb-32">
          ...
          <div className="flex items-center justify-between px-4 py-4 bg-[#121212]/50 border-b border-white/[0.05] mb-2">
            ...
            <button 
              ...
              className="flex items-center justify-center gap-1.5 px-3 py-2 bg-[#089981] text-black hover:bg-[#06806b] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-sm"
            >
              💬 Enter Chat
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/80 backdrop-blur-sm animate-fadeIn">
          ...
          <div className={`bg-[#1C1C1E] border-t ${displayToken.isGraduated ? 'border-amber-500/30' : (tradeMode === 'buy' ? 'border-[#089981]/30' : 'border-[#F23645]/30')} rounded-t-3xl w-full max-w-lg p-6 relative z-10 animate-slideUpNative flex flex-col transition-colors duration-300`}>
             ...
             <div className="bg-black/40 border border-white/5 focus-within:border-white/20 rounded-2xl p-4 flex items-center justify-between gap-4 transition-all mb-2">
                <input type="number" value={tradeAmount} onChange={(e) => setTradeAmount(e.target.value)} placeholder="0.00" className="bg-transparent text-left text-3xl font-black text-white w-full focus:outline-none placeholder-zinc-700 font-mono" />
             </div>

             <button 
                onClick={handleExecuteTrade}
                disabled={!tradeAmount || parseFloat(tradeAmount) <= 0}
                className={`w-full mt-4 ${tradeMode === 'buy' ? 'bg-[#089981] hover:bg-[#06806b]' : 'bg-[#F23645] hover:bg-rose-600'} disabled:opacity-50 text-white font-black text-sm py-4 rounded-2xl tracking-[0.2em] uppercase transition-all active:scale-95 flex items-center justify-center gap-2`}
             >
               Confirm {tradeMode} ⚡
             </button>
          </div>
      ...
      <div className="flex-none bg-[#0E0E14] z-30 pt-4 pb-24 md:pb-6 px-4 border-t border-white/[0.05] shadow-[0_-10px_30px_rgba(0,0,0,0.5)] relative">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          ...
          <button 
            ...
            className={`w-full ${displayToken.isGraduated ? 'bg-amber-500 hover:bg-amber-600 text-black' : 'bg-[#089981] hover:opacity-90 text-white'} font-black text-lg py-4 rounded-2xl flex items-center justify-center gap-2 transition-transform active:scale-95 uppercase tracking-widest`}
          >
