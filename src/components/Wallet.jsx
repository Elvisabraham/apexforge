...
// 🚀 FIXED: Added 'transactions', 'createdTokens', and 'onAddTransaction' to properties so it accepts live data
export default function Wallet({ setActivePage, onOpenProfile, onOpenSettings, onOpenAccountDrawer, portfolio = [], transactions = [], createdTokens = [], onAddTransaction, userProfile }) {
  ...
  const [viewAll, setViewAll] = useState({ assets: false, activity: false, created: false });
  ...
  // 🚀 FIXED: Removed the M and K abbreviations so your token amounts display exactly as you traded them.
  ...
  // 🚀 FIXED: Ensure SOL is always sorted to the top of the portfolio, and high value assets are next.
  ...
  const visibleActivity = viewAll.activity ? transactions : transactions.slice(0, 5);
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full h-screen overflow-hidden bg-[#030303] text-white selection:bg-[#089981]/30 font-sans relative">
        ...
        {/* --- HEADER --- */}
        <header className="flex-none z-40 bg-[#030303]/90 backdrop-blur-3xl px-4 sm:px-6 py-3 border-b border-white/[0.02] flex items-center justify-between sticky top-0">
          
          <div className="flex-1 flex items-center min-w-0">
            <div onClick={(e) => { e.stopPropagation(); setShowWalletManager(true); }} className="flex items-center gap-3 cursor-pointer group shrink-0">
               <div className="w-10 h-10 rounded-full border-2 border-white/5 overflow-hidden bg-[#121212] flex items-center justify-center shadow-inner group-hover:border-[#089981]/50 transition-colors">
                 {displayAvatar ? (
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    {connected ? (
                      <button 
                        ...
                        className="flex items-center gap-1.5 group/copy hover:bg-white/5 px-1.5 py-0.5 -ml-1.5 rounded-md transition-colors cursor-pointer"
                        title="Copy Address"
                      >
          ...
          <div className="flex-1 flex justify-center items-center pointer-events-none px-2">
            <div className={`transition-all duration-300 transform ${isScrolled ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
              <span className="text-sm font-black text-white tracking-tight">{isMuted ? '••••••' : displayNetWorth}</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex items-center justify-end gap-2 shrink-0">
            <button 
              ...
              className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-zinc-400 hover:text-white transition-all shadow-md group relative z-50 cursor-pointer"
            >
        ...
        >
          <div className="flex flex-col w-full max-w-4xl mx-auto">
            ...
            <section className="bg-gradient-to-b from-[#0A0A0A] to-[#050505] border-b border-white/[0.04] p-6 sm:p-8 flex flex-col items-center relative shadow-sm rounded-b-3xl mb-4">
              ...
              <h2 className={`text-5xl md:text-6xl font-sans tracking-tight font-bold text-white z-10 leading-none transition-opacity duration-300 ${isScrolled ? 'opacity-0' : 'opacity-100'}`}>
                  ...
                  {transactions.length > 0 ? (
                    <>
                      ...
                      {transactions.length > 5 && (
                        <button onClick={() => toggleViewAll('activity')} className="w-full py-3.5 mt-2 mb-8 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] text-xs font-black text-zinc-400 hover:text-white uppercase tracking-widest transition-colors shadow-sm">
                          {viewAll.activity ? 'View Less' : 'View Full Ledger'}
                        </button>
                      )}
      ...
      {/* 🚀 OVERLAYS OUTSIDE THE OVERFLOW CONTAINER */}
      
      {/* ------------------------------------------------------------- */}
      {/* --- THE LOCAL IDENTITY DRAWER (Fallback/Burner Manager) --- */}
      {/* ------------------------------------------------------------- */}
      {showWalletManager && (
      ...
      {/* ------------------------------------------------------------- */}
      {/* --- THE RIGHT-SIDE HAMBURGER MENU DRAWER ---                  */}
      {/* ------------------------------------------------------------- */}
      {isMenuOpen && (
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-[#000000]/80 backdrop-blur-3xl animate-fadeIn overflow-y-auto">
          <div className="bg-[#050505] border border-white/[0.05] rounded-[2.5rem] w-full max-w-[420px] p-6 sm:p-8 relative shadow-[0_0_80px_rgba(0,0,0,0.8)] flex flex-col h-auto max-h-[95vh] overflow-y-auto scrollbar-hide my-auto">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <div className="w-full bg-white/[0.02] border border-white/[0.04] rounded-2xl p-4 flex flex-col gap-4 shadow-lg backdrop-blur-sm">
                          <div className="flex justify-between items-center text-xs font-black">
                            <span className="text-zinc-500 uppercase tracking-[0.2em] text-[9px]">Network</span>
                            <span className="flex items-center gap-2 tracking-wider text-zinc-200"><span className="w-1.5 h-1.5 rounded-full bg-[#00FF66] shadow-[0_0_5px_#00FF66]"></span>Solana</span>
                          </div>
                          <div className="pt-3 border-t border-white/[0.02] flex flex-col gap-2">
                            <span className="text-[9px] text-zinc-500 font-black uppercase tracking-[0.2em]">Deposit Address</span>
                            <div className="flex justify-between items-center bg-black/50 p-3 rounded-xl border border-white/5">
                              <span className="text-xs font-mono text-zinc-300 truncate w-[70%] tracking-tight">{walletAddress.slice(0,10)}...</span>
                              <button onClick={handleCopyAddress} className="text-[#089981] font-black text-[10px] uppercase hover:text-white transition-colors tracking-wider">{copied ? 'Copied' : 'Copy'}</button>
                            </div>
                          </div>
                        </div>
                        <div className="w-full bg-[#3D1E16] border border-[#FF8A00] rounded-xl p-4 mt-6 flex items-center gap-3">
                          <svg className="w-5 h-5 text-[#FF8A00] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                          <span className="text-xs font-bold text-[#FF8A00]">Only send SOL on Solana.</span>
                        </div>
                      </div>
                    ) : (
                      <div className="flex flex-col flex-1 justify-between relative">
                        <div className="flex justify-center mb-8 relative z-20">
                          <button onClick={() => setShowFiatDropdown(!showFiatDropdown)} className="flex items-center gap-3 bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 hover:border-white/10 px-5 py-2.5 rounded-full transition-all duration-300 shadow-lg active:scale-95 backdrop-blur-md">
                            <span className="text-sm drop-shadow-sm">{activeFiatConfig?.icon}</span><span className="text-[10px] font-black text-zinc-200 uppercase tracking-[0.15em]">{activeFiatConfig?.label}</span><svg className={`w-3.5 h-3.5 text-zinc-500 transition-transform duration-300 ${showFiatDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                          </button>
                          {showFiatDropdown && (
                            <div className="absolute top-14 left-1/2 -translate-x-1/2 w-full max-w-[260px] bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.8)] animate-fadeIn z-50 p-1.5 flex flex-col gap-1 backdrop-blur-2xl">
                              {fiatProviders.map(provider => (
                                <button key={provider.id} onClick={() => { setSelectedFiatProvider(provider.id); setShowFiatDropdown(false); }} className={`w-full flex items-center gap-3 p-3.5 rounded-xl transition-all duration-200 text-left ${selectedFiatProvider === provider.id ? 'bg-[#089981]/20 border-transparent text-white shadow-inner' : 'hover:bg-white/5 text-zinc-400 hover:text-zinc-200 border border-transparent'}`}>
                                  <span className="text-base w-6 text-center drop-shadow-sm">{provider.icon}</span><span className="font-black text-[10px] uppercase tracking-[0.15em]">{provider.label}</span>{selectedFiatProvider === provider.id && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#089981] shadow-[0_0_5px_#089981]"></div>}
                                </button>
                              ))}
                            </div>
                          )}
                        ...
                        <div className="flex flex-col items-center justify-center py-4 mb-2">
                          <div className="flex items-baseline justify-center text-7xl font-sans tracking-tighter font-black select-none max-w-full overflow-hidden text-white drop-shadow-lg">
                            <span className="mr-1 text-zinc-400 font-medium">$</span><span>{depositAmount || '0'}</span>{depositAmount === '' && <span className="animate-pulse text-zinc-700 font-light ml-1">|</span>}
                          </div>
                        </div>
                        <div className="flex justify-center gap-2.5 mb-8">
                          {['30', '50', '100', '500'].map((amt) => (
                            <button key={amt} onClick={() => handleDepositQuickAmount(amt)} className="bg-white/[0.02] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 px-5 py-2 rounded-full font-black text-[11px] text-zinc-400 hover:text-white transition-all duration-300 shadow-md">${amt}</button>
                          ))}
                        </div>
                        <div className="grid grid-cols-3 gap-y-2 gap-x-4 mb-8 px-4">
                          {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0'].map((num) => (
                            <button key={num} onClick={() => handleNumpad(num)} className="text-2xl font-sans font-black text-zinc-200 hover:text-white hover:bg-white/[0.05] py-3.5 rounded-2xl active:scale-90 transition-all duration-200">{num}</button>
                          ))}
                          <button onClick={() => handleNumpad('back')} className="flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/[0.05] py-3.5 rounded-2xl active:scale-90 transition-all duration-200"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2M3 12l6.414 6.414a2 2 0 001.414.586H19a2 2 0 002-2V7a2 2 0 00-1.414.586L3 12z" /></svg></button>
                        </div>
                        <button onClick={handleExecuteDeposit} className="w-full bg-[#089981] hover:bg-[#06806b] text-white font-black uppercase text-[11px] py-4.5 rounded-2xl tracking-[0.2em] shadow-[0_0_20px_rgba(8,153,129,0.3)] transition-all duration-300 active:scale-95 mt-auto flex items-center justify-center gap-2">
                          Continue to {activeFiatConfig?.label || 'Payment'}
                        </button>
                      </div>
                    )}
                  </div>
            ...
            {/* --- SEND FLOW --- */}
            {activeModal === 'send' && (
              <div className="flex flex-col relative h-full animate-fadeIn mt-2">
                <div className="flex justify-center mb-8 relative z-20">
                  <div className="flex items-center gap-3 bg-white/[0.03] border border-white/5 px-5 py-2.5 rounded-full shadow-lg backdrop-blur-md">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black border bg-gradient-to-br from-blue-500/20 to-purple-500/20 text-blue-400 border-blue-500/20 shadow-inner">S</span>
                    <span className="text-[10px] font-black text-zinc-200 uppercase tracking-[0.15em]">SOL</span>
                  </div>
                </div>

                <div className="w-full bg-black/40 border border-white/5 focus-within:border-[#089981]/50 focus-within:bg-black/60 rounded-2xl p-2.5 flex items-center justify-between gap-2 transition-all duration-300 mb-4 shadow-inner">
                  <input type="text" value={sendAddress} onChange={(e) => setSendAddress(e.target.value)} placeholder="Recipient Solana Address..." className="bg-transparent text-xs text-zinc-200 w-full focus:outline-none px-3 font-mono tracking-tight placeholder-zinc-700" />
                  <button onClick={() => navigator.clipboard.readText().then(text => setSendAddress(text))} className="bg-white/10 hover:bg-white/20 text-zinc-300 hover:text-white text-[9px] font-black uppercase tracking-[0.2em] px-4 py-2.5 rounded-xl transition-all duration-300 shadow-sm">Paste</button>
                </div>

                <div className="flex flex-col items-center justify-center py-4 mb-2 min-w-0">
                  <div className="flex items-baseline justify-center text-7xl font-sans tracking-tighter font-black select-none max-w-full overflow-hidden text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-400 drop-shadow-lg w-full">
                    <span className="truncate block max-w-full text-center px-4">{sendAmount || '0'}</span>{sendAmount === '' && <span className="animate-pulse text-zinc-700 font-light ml-1 shrink-0">|</span>}
                  </div>
                  <span className="text-[10px] text-zinc-500 font-bold tracking-[0.15em] mt-3 uppercase">Available: <span className="text-zinc-300">{formatBalance(portfolio.find(a=>a.symbol==='SOL')?.balance)}</span> SOL</span>
                </div>

                <div className="flex justify-center gap-2.5 mb-8">
                  {['25%', '50%', '75%', 'MAX'].map((amt) => (
                    <button key={amt} onClick={() => handleCryptoPercentage(amt, true)} className="bg-white/[0.02] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 px-5 py-2 rounded-full font-black text-[11px] text-zinc-400 hover:text-white transition-all duration-300 shadow-md">{amt}</button>
                  ))}
                </div>
                ...
                <button onClick={handleExecuteSend} className="w-full bg-[#089981] hover:bg-[#06806b] text-white font-black uppercase text-[11px] py-4.5 rounded-2xl tracking-[0.2em] shadow-[0_0_20px_rgba(8,153,129,0.3)] transition-all duration-300 active:scale-95 mt-auto flex items-center justify-center gap-2">
                  Review Send
                </button>
              </div>
            )}
                  ...
                  <div className="flex flex-col animate-fadeIn h-full mt-2">
                    {activeWithdrawMethodConfig.type === 'crypto' ? (
                      <div className="flex flex-col flex-1 justify-between relative">
                        ...
                        <div className="w-full bg-black/40 border border-white/5 focus-within:border-[#089981]/50 focus-within:bg-black/60 rounded-2xl p-2.5 flex items-center justify-between gap-2 transition-all duration-300 mb-4 shadow-inner">
                          <input type="text" value={withdrawAddress} onChange={(e) => setWithdrawAddress(e.target.value)} placeholder="Destination Solana Address..." className="bg-transparent text-xs text-zinc-200 w-full focus:outline-none px-3 font-mono tracking-tight placeholder-zinc-700" />
                          <button onClick={() => navigator.clipboard.readText().then(text => setWithdrawAddress(text))} className="bg-white/10 hover:bg-white/20 text-zinc-300 hover:text-white text-[9px] font-black uppercase tracking-[0.2em] px-4 py-2.5 rounded-xl transition-all duration-300 shadow-sm">Paste</button>
                        </div>

                        <div className="flex flex-col items-center justify-center py-4 mb-2 min-w-0">
                          <div className="flex items-baseline justify-center text-7xl font-sans tracking-tighter font-black select-none max-w-full overflow-hidden text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-400 drop-shadow-lg w-full">
                            <span className="truncate block max-w-full text-center px-4">{withdrawAmount || '0'}</span>{withdrawAmount === '' && <span className="animate-pulse text-zinc-700 font-light ml-1 shrink-0">|</span>}
                          </div>
                          <span className="text-[10px] text-zinc-500 font-bold tracking-[0.15em] mt-3 uppercase">Available: <span className="text-zinc-300">{formatBalance(portfolio.find(a=>a.symbol==='SOL')?.balance)}</span> SOL</span>
                        </div>
                        ...
                        <div className="flex justify-center gap-2.5 mb-8">
                          {['25%', '50%', '75%', 'MAX'].map((amt) => (
                            <button key={amt} onClick={() => handleCryptoPercentage(amt, false)} className="bg-white/[0.02] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 px-5 py-2 rounded-full font-black text-[11px] text-zinc-400 hover:text-white transition-all duration-300 shadow-md">{amt}</button>
                          ))}
                        </div>
                        ...
                        <button onClick={handleExecuteWithdraw} className="w-full bg-[#089981] hover:bg-[#06806b] text-white font-black uppercase text-[11px] py-4.5 rounded-2xl tracking-[0.2em] shadow-[0_0_20px_rgba(8,153,129,0.3)] transition-all duration-300 active:scale-95 mt-auto flex items-center justify-center gap-2">
                          Review Transfer
                        </button>
                      </div>
                    ) : (
                      <div className="flex flex-col flex-1 justify-between relative">
                        <div className="flex justify-center mb-8 relative z-20">
                          <button onClick={() => setShowWithdrawFiatDropdown(!showWithdrawFiatDropdown)} className="flex items-center gap-3 bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 hover:border-white/10 px-5 py-2.5 rounded-full transition-all duration-300 shadow-lg active:scale-95 backdrop-blur-md">
                            <span className="text-sm drop-shadow-sm">{activeWithdrawFiatConfig?.icon}</span><span className="text-[10px] font-black text-zinc-200 uppercase tracking-[0.15em]">{activeWithdrawFiatConfig?.label}</span><svg className={`w-3.5 h-3.5 text-zinc-500 transition-transform duration-300 ${showWithdrawFiatDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                          </button>
                          {showWithdrawFiatDropdown && (
                            <div className="absolute top-14 left-1/2 -translate-x-1/2 w-full max-w-[260px] bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.8)] animate-fadeIn z-50 p-1.5 flex flex-col gap-1 backdrop-blur-2xl">
                              {withdrawFiatProviders.map(provider => (
                                <button key={provider.id} onClick={() => { setSelectedWithdrawFiatProvider(provider.id); setShowWithdrawFiatDropdown(false); }} className={`w-full flex items-center gap-3 p-3.5 rounded-xl transition-all duration-200 text-left ${selectedWithdrawFiatProvider === provider.id ? 'bg-[#089981]/20 border-transparent text-white shadow-inner' : 'hover:bg-white/5 text-zinc-400 hover:text-zinc-200 border border-transparent'}`}>
                                  <span className="text-base w-6 text-center drop-shadow-sm">{provider.icon}</span><span className="font-black text-[10px] uppercase tracking-[0.15em]">{provider.label}</span>{selectedWithdrawFiatProvider === provider.id && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#089981] shadow-[0_0_5px_#089981]"></div>}
                                </button>
                              ))}
                            </div>
                          )}
                        ...
                        <div className="flex flex-col items-center justify-center py-4 mb-2 min-w-0">
                          <div className="flex items-baseline justify-center text-7xl font-sans tracking-tighter font-black select-none max-w-full overflow-hidden text-white drop-shadow-lg w-full">
                            <span className="mr-1 text-zinc-400 font-medium shrink-0">$</span><span className="truncate block max-w-full text-center">{withdrawAmount || '0'}</span>{withdrawAmount === '' && <span className="animate-pulse text-zinc-700 font-light ml-1 shrink-0">|</span>}
                          </div>
                        </div>
                        <div className="flex justify-center gap-2.5 mb-8">
                          {['30', '50', '100', '500'].map((amt) => (
                            <button key={amt} onClick={() => handleWithdrawQuickAmount(amt)} className="bg-white/[0.02] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 px-5 py-2 rounded-full font-black text-[11px] text-zinc-400 hover:text-white transition-all duration-300 shadow-md">${amt}</button>
                          ))}
                        </div>
                        <div className="grid grid-cols-3 gap-y-2 gap-x-4 mb-8 px-4">
                          {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0'].map((num) => (
                        ...
                        <button onClick={handleExecuteWithdraw} className="w-full bg-[#089981] hover:bg-[#06806b] text-white font-black uppercase text-[11px] py-4.5 rounded-2xl tracking-[0.2em] shadow-[0_0_20px_rgba(8,153,129,0.3)] transition-all duration-300 active:scale-95 mt-auto flex items-center justify-center gap-2">
                          Withdraw to {activeWithdrawFiatConfig?.label || 'Payment'}
                        </button>
                      </div>
                    )}
                  </div>
            ...
            {activeModal === 'swap' && (
              <div className="flex flex-col relative h-full animate-fadeIn mt-2">
                ...
                <div className="bg-white/[0.02] border border-white/[0.05] rounded-[1.5rem] p-5 shadow-lg relative group focus-within:border-[#089981]/30 transition-all duration-300">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.15em]">You Pay</span>
                    <span className="text-[10px] text-zinc-500 font-bold">Balance: {formatBalance(activePayAsset?.balance)}</span>
                  </div>
                  
                  <div className="flex justify-between items-center gap-4">
                    <div className="relative shrink-0">
                      <button onClick={() => setShowPayDropdown(!showPayDropdown)} className="flex items-center gap-2 bg-black/40 hover:bg-black/60 border border-white/5 hover:border-white/10 px-4 py-2 rounded-xl transition-all duration-300">
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border overflow-hidden shadow-inner ${activePayAsset?.imagePreview ? 'bg-black border-white/10' : `bg-gradient-to-br ${getAssetColor(activePayAsset?.symbol || '')}`}`}>
                          {activePayAsset?.imagePreview ? <img src={activePayAsset.imagePreview} className="w-full h-full object-cover" /> : activePayAsset?.icon}
                        </span>
                        <span className="text-sm font-black text-white tracking-wide uppercase">{activePayAsset?.symbol}</span>
                        <svg className={`w-3.5 h-3.5 text-zinc-500 transition-transform duration-300 ${showPayDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                      </button>

                      {showPayDropdown && (
                        <div className="absolute top-12 left-0 w-[180px] bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.8)] animate-fadeIn z-50 p-1.5 flex flex-col gap-1 backdrop-blur-2xl">
                          {portfolio.map(asset => (
                            <button key={asset.symbol} onClick={() => { setSwapPayAsset(asset.symbol); setShowPayDropdown(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-all duration-200 text-left">
                              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border overflow-hidden shadow-inner ${asset.imagePreview ? 'bg-black border-white/10' : `bg-gradient-to-br ${getAssetColor(asset.symbol || '')}`}`}>
                                {asset.imagePreview ? <img src={asset.imagePreview} className="w-full h-full object-cover" /> : asset.icon}
                              </span>
                              <span className="font-black text-xs uppercase tracking-wide text-zinc-300 hover:text-white">{asset.symbol}</span>
                            </button>
                          ))}
                        </div>
                      )}
                    ...
                    <div className="flex flex-col items-end min-w-0 flex-1">
                      <div className="flex items-baseline text-3xl font-sans tracking-tight font-black text-white w-full justify-end overflow-hidden">
                        <span className="truncate block max-w-full text-right">{swapAmount || '0'}</span>
                        {swapAmount === '' && <span className="animate-pulse text-zinc-700 font-light ml-1 shrink-0">|</span>}
                      </div>
                      <span className="text-[10px] text-zinc-500 font-bold mt-1">~ $0.00</span>
                    </div>
                  </div>
                </div>

                <div className="flex justify-center -my-3 z-10 relative">
                  <button onClick={flipSwap} className="bg-[#050505] p-2 rounded-full border border-white/10 hover:border-[#089981]/50 hover:scale-110 hover:shadow-[0_0_20px_rgba(8,153,129,0.2)] transition-all duration-300 group">
                    <div className="bg-white/[0.05] p-2 rounded-full group-hover:bg-[#089981]/20 transition-colors">
                      <svg className="w-4 h-4 text-zinc-400 group-hover:text-[#089981] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4-4l4-4" /></svg>
                    </div>
                  </button>
                </div>

                <div className="bg-white/[0.02] border border-white/[0.05] rounded-[1.5rem] p-5 shadow-lg relative group transition-all duration-300 mb-6">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.15em]">You Receive</span>
                    <span className="text-[10px] text-zinc-500 font-bold">Balance: {formatBalance(activeReceiveAsset?.balance)}</span>
                  </div>
                  
                  <div className="flex justify-between items-center gap-4">
                    <div className="relative shrink-0">
                      <button onClick={() => setShowReceiveDropdown(!showReceiveDropdown)} className="flex items-center gap-2 bg-black/40 hover:bg-black/60 border border-white/5 hover:border-white/10 px-4 py-2 rounded-xl transition-all duration-300">
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border overflow-hidden shadow-inner ${activeReceiveAsset?.imagePreview ? 'bg-black border-white/10' : `bg-gradient-to-br ${getAssetColor(activeReceiveAsset?.symbol || '')}`}`}>
                          {activeReceiveAsset?.imagePreview ? <img src={activeReceiveAsset.imagePreview} className="w-full h-full object-cover" /> : activeReceiveAsset?.icon}
                        </span>
                        <span className="text-sm font-black text-white tracking-wide uppercase">{activeReceiveAsset?.symbol}</span>
                        <svg className={`w-3.5 h-3.5 text-zinc-500 transition-transform duration-300 ${showReceiveDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                      </button>

                      {showReceiveDropdown && (
                        <div className="absolute top-12 left-0 w-[180px] bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.8)] animate-fadeIn z-50 p-1.5 flex flex-col gap-1 backdrop-blur-2xl">
                          {portfolio.map(asset => (
                            <button key={asset.symbol} onClick={() => { setSwapReceiveAsset(asset.symbol); setShowReceiveDropdown(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-all duration-200 text-left">
                              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border overflow-hidden shadow-inner ${asset.imagePreview ? 'bg-black border-white/10' : `bg-gradient-to-br ${getAssetColor(asset.symbol || '')}`}`}>
                                {asset.imagePreview ? <img src={asset.imagePreview} className="w-full h-full object-cover" /> : asset.icon}
                              </span>
                              <span className="font-black text-xs uppercase tracking-wide text-zinc-300 hover:text-white">{asset.symbol}</span>
                    ...
                    <div className="flex flex-col items-end min-w-0 flex-1">
                      <div className="flex items-baseline text-3xl font-sans tracking-tight font-black text-zinc-400 w-full justify-end overflow-hidden">
                        <span className="truncate block max-w-full text-right">
                          {swapAmount 
                            ? formatNumber((parseFloat(swapAmount.replace(/,/g, '')) * 1.5).toFixed(2)) 
                            : '0.00'}
                        </span>
                      </div>
                      <span className="text-[10px] text-zinc-500 font-bold mt-1">~ $0.00</span>
                ...
                <button onClick={handleExecuteSwap} className="w-full bg-[#089981] hover:bg-[#06806b] text-white font-black uppercase text-[11px] py-4.5 rounded-2xl tracking-[0.2em] shadow-[0_0_20px_rgba(8,153,129,0.3)] transition-all duration-300 active:scale-95 mt-auto flex items-center justify-center gap-2">
                  Review Swap
                </button>
              </div>
            )}
      ...
      {/* ------------------------------------------------------------- */}
      {settingsView && (
        <div style={{ zIndex: 9999 }} className="fixed inset-0 bg-black/80 backdrop-blur-md animate-fadeIn flex justify-center items-center">
          <AccountSettingsSystem 
            initialView={settingsView} 
            onCloseSettings={() => setSettingsView(null)} 
