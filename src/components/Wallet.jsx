...
export default function Wallet({ setActivePage, onOpenProfile, onOpenSettings, onOpenAccountDrawer, portfolio = [], transactions = [], createdTokens = [], onAddTransaction, userProfile }) {
  ...
  const getAssetColor = (symbol) => {
    if (symbol === 'SOL') return 'from-blue-500/20 to-[#089981]/20 text-blue-400 border-blue-500/20';
    const colors = [
      'from-[#089981]/20 to-emerald-500/20 text-[#089981] border-[#089981]/20',
      'from-teal-500/20 to-cyan-500/20 text-teal-400 border-teal-500/20',
      'from-amber-500/20 to-orange-500/20 text-amber-400 border-amber-500/20',
      'from-emerald-500/20 to-teal-600/20 text-emerald-400 border-emerald-500/20'
    ];
    ...
    <>
      <div className="flex flex-col w-full h-screen overflow-hidden bg-[#030303] text-white selection:bg-[#089981]/30 font-sans relative">
        ...
        <header className="flex-none z-40 bg-[#030303]/90 backdrop-blur-3xl px-4 sm:px-6 py-3 border-b border-white/[0.02] flex items-center justify-between sticky top-0 relative overflow-hidden">
          ...
          {/* LEFT: Profile OR Balance (They share the exact same left-aligned space) */}
          <div className="flex-1 relative h-12 flex items-center min-w-0 z-10 mr-4">
                ...
                <div className="flex items-center mt-0.5 w-full min-w-0">
                  {connected ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 group/copy hover:bg-white/5 py-1 px-1.5 -ml-1.5 rounded-lg transition-colors cursor-pointer max-w-fit min-w-0 shrink-0 overflow-visible"
                      title="Copy Address"
                    >
                      ...
                      <div className="flex-none w-[14px] h-[14px] flex items-center justify-center">
                        {copied ? (
                          ...
                          <svg className="w-full h-full text-zinc-500 group-hover/copy:text-[#089981] transition-colors" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    ...
                    <span className="text-[9px] text-zinc-500 font-black uppercase tracking-widest truncate mt-0.5 min-w-0">Disconnected</span>
                  )}
                </div>
              </div>
            </div>

            {/* 2. Scrolled Balance (Left Aligned exactly where the avatar was) */}
            <div className={`absolute inset-0 flex items-center transition-all duration-300 pointer-events-none ${isScrolled ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
          ...
          {/* RIGHT: Menu Button */}
          <div className="flex items-center justify-end z-10 shrink-0 w-12 ml-auto">
            <button
        ...
        <div
          className={`fixed top-20 left-1/2 -translate-x-1/2 z-[100] transition-all duration-300 ease-out ${copied ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-4 scale-95 pointer-events-none'
            }`}
        >
              ...
              <div className="flex bg-[#121212] border border-white/5 p-1 rounded-2xl w-full max-w-[420px] mx-auto">
                {['assets', 'activity', 'created'].map((tab) => (
                  <button
                    ...
                    className={`flex-1 py-2.5 text-[11px] font-black uppercase tracking-[0.15em] rounded-xl transition-all duration-300 ${activeTab === tab
                ...
                <div className="flex flex-col gap-4 animate-fadeIn">
                  {createdTokens.length > 0 ? (
                    <>
                      {visibleCreated.map((token, index) => (
                        <div key={index} onClick={() => setActivePage('tokenHome')} className="p-5 bg-[#0A0A0A] border border-white/[0.03] rounded-2xl hover:border-[#089981]/30 transition-all duration-300 shadow-lg flex flex-col gap-4 group cursor-pointer">
                          <div className="flex justify-between items-center">
                            ...
                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border shrink-0 ${token.isGraduated ? 'text-[#089981] bg-[#089981]/10 border-[#089981]/20' : 'text-amber-400 bg-amber-400/10 border-amber-400/20'}`}>
                          ...
                          <div className="grid grid-cols-2 gap-3 pt-3 border-t border-white/[0.03]">
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              <span className="text-xs font-black text-[#089981] mt-0.5 drop-shadow-[0_0_8px_rgba(8,153,129,0.3)]">{token.royalties || '0.00 SOL'}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-4 mb-8">
                        <button onClick={() => setActivePage && setActivePage('explore')} className="w-full flex items-center justify-center gap-2 py-4 bg-[#121212] hover:bg-zinc-800/80 border border-white/10 hover:border-white/20 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl transition-all duration-300 active:scale-95 shadow-lg">
                          Explore 🔍
                        </button>
                        <button onClick={() => setActivePage && setActivePage('earn')} className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-[#089981] to-emerald-600 hover:from-emerald-500 hover:to-emerald-700 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-[0_10px_30px_rgba(8,153,129,0.2)] hover:shadow-[0_15px_40px_rgba(8,153,129,0.4)] transition-all duration-300 active:scale-95">
                          Earn Yield 💰
